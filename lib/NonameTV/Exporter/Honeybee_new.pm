package NonameTV::Exporter::Honeybee_new;
...
sub ReadState {
  ...
  my $last_update = $ds->sa->Lookup( 'state', { name => "honeybee_new_last_update" },
                                 'value' );

  if( not defined( $last_update ) )
  {
    $ds->sa->Add( 'state', { name => "honeybee_new_last_update", value => 0 } );
    $last_update = 0;
  }
...
sub WriteState {
  ...
  $ds->sa->Update( 'state', { name => "honeybee_new_last_update" },
