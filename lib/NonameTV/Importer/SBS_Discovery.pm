package NonameTV::Importer::SBS_Discovery;
...
Imports data for Kanal 5, Kanal 9 and Kanal 11.
...
use NonameTV qw/norm ParseDescCatSwe AddCategory/;
...
sub ImportXML
{
  ...
  progress( "SBS_Discovery: $chd->{xmltvid}: Processing XML $file" );
  ...
  if( not defined( $doc ) ) {
    error( "SBS_Discovery: $file: Failed to parse xml" );
    return;
  }
  ...
  if( $rows->size() == 0 ) {
    error( "SBS_Discovery: $chd->{xmltvid}: No Rows found" ) ;
    return;
  }
  ...
  if(!defined $year) {
    error( "SBS_Discovery: $chd->{xmltvid}: Failure to get year from filename" ) ;
    return;
  } else { $year += 2000; }
  ...
  foreach my $row ($rows->get_nodelist) {
    ...
    # Grab title from @title in the transmission if no title can be found
    $title = $row->findvalue( './@title' ) if !defined($title);
    ...
    my($program_type, $category ) = $ds->LookupCat( 'SBS_Discovery', $genre );
