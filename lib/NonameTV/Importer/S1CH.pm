package NonameTV::Importer::S1CH;
...
sub ImportContentFile
{
  ...
  foreach my $program (sort by_start $programs->get_nodelist) {
        ...
        if($start->ymd("-") ne $currdate ) {
          ...
          $currdate = $start->ymd("-");
          progress("S1CH: $chd->{xmltvid}: Date is: ".$start->ymd("-"));
        }
        ...
        if( $genre ){
          my ( $program_type, $category ) = $self->{datastore}->LookupCat( "S1CH_genre", $genre );
          AddCategory( $ce, $program_type, $category );
        }
        $genre = $xpc->findvalue( 's:infos/s:klassifizierung/@formatgruppe' );
        if( $genre ){
          my ( $program_type2, $category2 ) = $self->{datastore}->LookupCat( "S1CH_main", $genre );
          AddCategory( $ce, $program_type2, $category2 );
        }

        my ($folge, $staffel);
        my $folgennummer = $xpc->findvalue( 's:infos/s:folge/@folgennummer' );
        ...
        # Staffel + episode
        if(defined($folgennummer) and $folgennummer ne "") {
          if(($staffel, $folge) = ($folgennummer =~ /(\d+)\.(\d+)/)) {
            $ce->{episode} = ($staffel - 1) . ' . ' . ($folge - 1) . ' .';
          } else {
            $ce->{episode} =  ' . ' . ($folgennummer - 1) . ' .';
          }
        ...
        progress("S1CH: $chd->{xmltvid}: ".$ce->{start_time}." - ".$ce->{title});
  }
...
sub ParseCredits
{
  ...
  if( $field eq "directors" and scalar( @people ) > 0 ) {
