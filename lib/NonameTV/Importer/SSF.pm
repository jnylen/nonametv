...
sub ImportXLS {
  ...
  progress( "SSF: $xmltvid: Processing $file" );
...
my $ref = ReadData ($file);
  ...
  for(my $iSheet=0; $iSheet < $oBook->{SheetCount} ; $iSheet++) {
    ...
    progress( "SSF: Processing worksheet: $oWkS->{Name}" );
    ...
    $i++;
      ...
      # Extra
      $oWkC = $oWkS->{Cells}[$iR][$colextra];
      next if( ! $oWkC );
      my $extra = $oWkC->Value if( $oWkC->Value );

      if(defined($extra)) {
          my $episode = ($extra =~ /Episode (\d+)/i);
          my $season = ($extra =~ /Staffel (\d+)/i);

          # Program type and episode info
          if(defined $episode and $episode ne "") {
	  ...
	  progress("SSF: $time - $title") if $title;
