package NonameTV::Importer::SkyNews;
...
Channels: Sky News International
...
sub ImportContentFile {
  ...
  if( $file =~ /\.xls$/i ){
    ...
    error( "SkyNews: Unknown file format: $file" );
  }
...
sub ImportXLS
{
  ...
  progress("SkyNews: Processing $file");
  ...
  foreach my $oWkS (@{$oBook->{Worksheet}}) {
    ...
    progress( "SkyNews: $chd->{xmltvid}: Processing worksheet: $oWkS->{Name}" );
    ...
    for(my $iR = 0 ; defined $oWkS->{MaxRow} && $iR <= $oWkS->{MaxRow} ; $iR++) {
      ...
      if( $date ne $currdate ) {
        ...
        progress("SkyNews: $chd->{xmltvid}: Date is $date");
      ...
      progress("SkyNews: $chd->{xmltvid}: $time - $title");
