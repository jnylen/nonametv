package NonameTV::Importer::Classica;
...
sub ImportXLS {
  ...
  progress( "Classica: $xmltvid: Processing $file" );
...
my $ref = ReadData ($file);
  ...
  for(my $iSheet=0; $iSheet < $oBook->{SheetCount} ; $iSheet++) {
    ...
    progress( "Classica: Processing worksheet: $oWkS->{Name}" );
    ...
    $i++;
      ...
      if( $date ne $currdate ){
        ...
        progress("Classica: Date is $date");
      ...
      # year
      $oWkC = $oWkS->{Cells}[$iR][$coltime];
      my $year = $oWkC->{Val} if( $oWkC->Value );

      if (defined $year and $year ne "") {
        $ce->{production_date} = $year."-01-01";
      }

	  progress("Kanal10NO: $time - $title") if $title;
