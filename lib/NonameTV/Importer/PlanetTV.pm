package NonameTV::Importer::PlanetTV;
...
sub ImportContentFile
{
  ...
  foreach my $pgm (sort by_start $programs->get_nodelist) {
    ...
    #$ce->{end_time} = $end->ymd("-") . "T" . $end->hms(":");

    if($start->ymd("-") ne $currdate ) {
      $dsh->StartDate( $start->ymd("-") , "06:00" );
      $currdate = $start->ymd("-");
      progress("PlanetTV: $chd->{xmltvid}: Date is: ".$start->ymd("-"));
    }
    ...
    my $production_year = $xpc->findvalue( 's:infos/s:produktion[@gueltigkeit="gesamt"]/s:produktionszeitraum/s:jahr/@von' );
    ...
    if( $genre ){
      my ( $program_type, $category ) = $self->{datastore}->LookupCat( "PlanetTV_genre", $genre );
      AddCategory( $ce, $program_type, $category );
    }
    $genre = $xpc->findvalue( 's:infos/s:klassifizierung/@formatgruppe' );
    if( $genre ){
      my ( $program_type2, $category2 ) = $self->{datastore}->LookupCat( "PlanetTV_main", $genre );
      AddCategory( $ce, $program_type2, $category2 );
    }

    #Descr
    my $desc = $xpc->findvalue( 's:text[@textart="Beschreibung"]' );
    if( ! $desc) {
    ...
    progress("PlanetTV: $chd->{xmltvid}: ".$ce->{start_time}." - ".$ce->{title});
